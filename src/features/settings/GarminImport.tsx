...
export function GarminImport() {
  const addCardioMany = useMettlefieldStore((state) => state.addCardioMany); const addSleep = useMettlefieldStore((state) => state.addSleep); const [message, setMessage] = useState("");
  async function importFiles(event: ChangeEvent<HTMLInputElement>) { let activities = 0; let nights = 0; const warnings: string[] = []; for (const file of Array.from(event.target.files || [])) { const result = parseGarminExport(await file.text(), file.name); addCardioMany(result.cardio); result.sleep.forEach(addSleep); activities += result.cardio.length; nights += result.sleep.length; warnings.push(...result.warnings); } setMessage(warnings[0] || `Imported ${activities} activities and ${nights} sleep records.`); event.target.value = ""; }
